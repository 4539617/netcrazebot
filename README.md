...
Многофункциональный Telegram бот для работы с сетевыми конфигурациями.

## Описание

Бот предоставляет три основные функции:
1. **Генерация .bat файлов** - разрешает DNS для доменов и создает Windows batch файлы с командами маршрутизации
2. **Обработка .bat файлов** - добавляет комментарии к существующим .bat файлам через reverse DNS и IP info API
3. **Конвертация AmneziaWG** - преобразует конфигурационные файлы AmneziaWG из версии 2 в версию 1

## Возможности

### 1. Генерация .bat файлов для доменов
...
### 2. Обработка .bat файлов
- ✅ Парсинг команд `route add` из .bat файлов
- ✅ Добавление комментариев через reverse DNS lookup
- ✅ Использование IP info API (ip-api.com) как резервный метод
- ✅ Извлечение основных доменов (например, `yandex.net` из `kp-nginx.kp.yandex.net`)
- ✅ Поддержка российских доменов (`.gov.ru`, `.edu.ru`, `.com.ru` и др.)
- ✅ Детальная статистика обработки

### 3. Конвертация AmneziaWG конфигураций
- ✅ Автоматическое определение версии конфигурации
- ✅ Конвертация H-параметров из диапазонов в фиксированные значения
- ✅ Удаление параметров S3 и S4 (не используются в v1)
- ✅ Сохранение всех остальных параметров

### Общие возможности
- ✅ Защита от флуда (10 запросов в минуту на пользователя)
...
1. Отправьте боту .bat файл с командами `route add`
2. Бот проанализирует файл и добавит комментарии к IP-адресам
3. Комментарии добавляются через:
   - Reverse DNS lookup (с проверкой соответствия)
   - IP info API (ip-api.com) для IP без PTR записей
4. Основные домены извлекаются автоматически:
   - `kp-nginx.kp.yandex.net` → `yandex.net`
   - `server.site.gov.ru` → `site.gov.ru`
5. Бот отправит обработанный файл со статистикой
...
До обработки:
✅ С комментариями: 3
❌ Без комментариев: 170

После обработки:
➕ Добавлено комментариев: 143
🎯 Итого с комментариями: 146
⚠️ Осталось без комментариев: 27
```

### 3. Конвертация AmneziaWG конфигураций

1. Отправьте боту .conf файл AmneziaWG (версия 2)
2. Бот автоматически определит версию конфигурации
3. Если это версия 2, бот конвертирует её в версию 1:
   - H-параметры: диапазоны → фиксированные значения
   - Удаление параметров S3 и S4
4. Бот отправит конвертированный файл

**⚠️ Важно:** После конвертации проверьте работоспособность конфигурации!

**Пример конвертации H-параметров:**
```
v2: H1 = 1726271876-1813116022
v1: H1 = 1726271876

v2: H2 = 1831845225-2080655774
v1: H2 = 1831845225
```
...
│   ├── fileGenerator.js     # Генерация .bat файлов для доменов
│   ├── batFileProcessor.js  # Обработка .bat файлов (добавление комментариев)
│   ├── awgConverter.js      # Конвертация AmneziaWG конфигураций
